...
var dom = domQuery.createScope({
  ...
  getTagElements: (ctx) => domQuery.queryAll(dom.getRootEl(ctx), `[data-part=item-preview]:not([data-disabled])`),
  getFirstEl: (ctx) => dom.getTagElements(ctx)[0],
  getLastEl: (ctx) => dom.getTagElements(ctx)[dom.getTagElements(ctx).length - 1],
  getPrevEl: (ctx, id) => domQuery.prevById(dom.getTagElements(ctx), id, false),
  getNextEl: (ctx, id) => domQuery.nextById(dom.getTagElements(ctx), id, false),
  getTagElAtIndex: (ctx, index) => dom.getTagElements(ctx)[index],
  getIndexOfId: (ctx, id) => domQuery.indexOfId(dom.getTagElements(ctx), id),
    ...
    },
    {
      ...
      activities: {
        trackInteractOutside(ctx2, _evt, { send }) {
          return interactOutside.trackInteractOutside(dom.getInputEl(ctx2), {
            exclude(target) {
              return domQuery.contains(dom.getRootEl(ctx2), target);
            },
      ...
      actions: {
        ...
        deleteTag(ctx2, evt) {
          ...
          set.value(ctx2, utils.removeAt(ctx2.value, index));
        },
        ...
        addTag(ctx2, evt) {
          ...
          if (guard) {
            const nextValue = utils.uniq(ctx2.value.concat(value));
        ...
        addTagFromPaste(ctx2) {
          domQuery.raf(() => {
            ...
            if (guard) {
              ...
              const nextValue = utils.uniq(ctx2.value.concat(...trimmedValue));
              set.value(ctx2, nextValue);
              ctx2.log.prev = ctx2.log.current;
...
var set = {
  ...
  highlightedId: (ctx, id) => {
    if (utils.isEqual(ctx.highlightedTagId, id)) return;
